...
public class UploadServletController extends HttpServlet {
	...
	private static final ResourceBundle res = ResourceBundle.getBundle(Constants.IMG_CONFIG_PATH);
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		extMap.put("image", Constants.IMAGE_TYPE);
		extMap.put("flash", Constants.FLASH_TYPE);
		extMap.put("media", Constants.MEDIA_TYPE);
		extMap.put("file", Constants.FILE_TYPE);
		...
		try {
			...
			while (itr.hasNext()) {
				...
				if (!item.isFormField()) {
					// 检查文件大小
					if (item.getSize() > Constants.MAX_SIZE) {
